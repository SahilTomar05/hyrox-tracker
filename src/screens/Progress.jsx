...
// Estimate 1RM from reps x weight (Epley formula)
...
function ShareModal({ profile, stats, onClose }) {
  ...
  const scoreColor = stats.consistencyScore >= 80 ? '#22C55E'
    : stats.consistencyScore >= 60 ? '#FF5A1F'
    : '#EF4444'
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.88)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}
      ...
      <div style={{width:'100%',maxWidth:360,display:'flex',flexDirection:'column',gap:14}}
        ...
        {/* Card */}
        <div style={{background:'linear-gradient(160deg,#0d0d0d,#111)',border:'1px solid #FF5A1F25',borderRadius:28,padding:24}}>
          {/* Header */}
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
            ...
            {daysLeft&&daysLeft>0&&(
              <div style={{background:'#FF5A1F15',border:'1px solid #FF5A1F30',borderRadius:10,padding:'4px 12px'}}>
                <span style={{fontSize:12,color:'#FF5A1F',fontWeight:600}}>{daysLeft}d to go 🏁</span>
              </div>
            )}
          </div>

          {/* Name */}
          <p style={{fontSize:11,color:'#444',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:4}}>Weekly Update</p>
          <h2 style={{fontSize:26,fontWeight:700,color:'#fff',marginBottom:4}}>{profile?.name}</h2>
          {profile?.event_name&&<p style={{fontSize:12,color:'#FF5A1F',marginBottom:16}}>{profile.event_name}</p>}
          <p style={{fontSize:11,color:'#333',marginBottom:20}}>{new Date().toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</p>

          {/* Consistency hero */}
          <div style={{background:scoreColor+'10',border:`1px solid ${scoreColor}25`,borderRadius:18,padding:20,textAlign:'center',marginBottom:16}}>
            <p style={{fontSize:11,color:'#444',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:8}}>Weekly Consistency Score</p>
            <div style={{fontSize:68,fontWeight:700,color:scoreColor,lineHeight:1,marginBottom:4}}>{stats.consistencyScore}</div>
            <p style={{fontSize:13,color:scoreColor,fontWeight:600}}>
              {stats.consistencyScore>=80?'Crushing it 🔥':stats.consistencyScore>=60?'On track 💪':stats.consistencyScore>=40?'Building momentum':'Time to level up'}
            </p>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
            {[
              {label:'Sessions this week',val:stats.sessionsThisWeek,icon:'💪',color:'#FF5A1F'},
              {label:'Avg kcal/day',val:stats.avgCalories,icon:'🔥',color:'#EF4444'},
              {label:'Avg protein/day',val:`${stats.avgProtein}g`,icon:'🥩',color:'#A855F7'},
              {label:'Avg steps/day',val:stats.avgSteps.toLocaleString(),icon:'👟',color:'#22C55E'},
            ].map(({label,val,icon,color})=>(
              <div key={label} style={{background:'#ffffff08',borderRadius:14,padding:'12px 14px'}}>
                <p style={{fontSize:20,fontWeight:700,color,lineHeight:1,marginBottom:4}}>{val}</p>
                <p style={{fontSize:11,color:'#444'}}>{icon} {label}</p>
              </div>
            ))}
          </div>
          ...
          {stats.topImprovement&&(
            <div style={{background:'#FF5A1F10',border:'1px solid #FF5A1F20',borderRadius:14,padding:'12px 16px',marginBottom:16}}>
              <p style={{fontSize:10,color:'#555',textTransform:'uppercase',letterSpacing:'.08em',marginBottom:4}}>🏆 Top improvement</p>
              <p style={{fontSize:14,fontWeight:600,color:'#fff'}}>{stats.topImprovement.exercise}</p>
              <p style={{fontSize:13,color:'#FF5A1F',fontWeight:700}}>+{stats.topImprovement.pct}% this week</p>
            </div>
          )}
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          ...
          <button onClick={onClose}
            style={{background:'#111',border:'1px solid #1e1e1e',borderRadius:14,padding:14,color:'#666',fontSize:14,fontWeight:600,cursor:'pointer'}}>
            Close
          </button>
        </div>
...
export default function Progress({ session, profile }) {
  ...
  async function fetchAll() {
    setLoading(true)
    const [sRes, wRes, nRes] = await Promise.all([
  ...
  // Calories burned per day (last 7)
  const caloriesByDay = last7Days.map(date => {
    const dayNutrition = nutritionLogs.find(n => {
      const nd = new Date(n.date)
      return nd.toDateString() === date.toDateString()
    })
    const cals = dayNutrition ? (dayNutrition.meals||[]).reduce((s,m)=>s+Number(m.calories||0),0) : 0
    return { date: shortDay(date), calories: Math.round(cals), isToday: date.toDateString()===new Date().toDateString() }
  })
  ...
  // Strength progress — extract 1RM per exercise
  const strengthExercises = sessions.flatMap(s =>
  ...
  // Group by exercise name → latest 1RM + delta
  const exerciseNames = [...new Set(strengthExercises.map(e=>e.name))]
  const strengthPRs = exerciseNames.map(name => {
    const logs = strengthExercises.filter(e=>e.name===name).sort((a,b)=>new Date(a.date)-new Date(b.date))
    const latest = logs[logs.length-1]?.bestSet || 0
    const prev = logs.length > 1 ? logs[logs.length-2]?.bestSet : null
    const delta = prev ? ((latest-prev)/prev*100).toFixed(1) : null
    return { name, rm: latest, delta: delta ? Number(delta) : null, count: logs.length }
  }).filter(e=>e.rm>0).sort((a,b)=>b.rm-a.rm)

  // Exercise chart data
  const exerciseChartData = selectedExercise
    ? strengthExercises.filter(e=>e.name===selectedExercise)
        .sort((a,b)=>new Date(a.date)-new Date(b.date))
        .slice(-10)
        .map(e=>({date:dayLabel(e.date),value:e.bestSet}))
    : []
  ...
  const nutritionDays = last7Nutrition.filter(n=>{
  ...
  const consistencyScore = Math.min(Math.round((sessionDays/7)*40+(nutritionDays/7)*35+(stepsDays/7)*25),100)

  // Top improvement this week
  const thisWeekExercises = thisWeekSessions.flatMap(s=>
    (s.exercises||[]).filter(e=>e.fieldType==='strength').map(ex=>({
      name: ex.name,
      bestSet: (ex.sets||[]).reduce((best,set)=>{
        const rm=calc1RM(set.reps,set.weight); return rm>best?rm:best
      },0)
    }))
  )
  const topImprovement = (() => {
    const improvements = exerciseNames.map(name=>{
      const all = strengthExercises.filter(e=>e.name===name).sort((a,b)=>new Date(a.date)-new Date(b.date))
      const thisWeek = thisWeekExercises.filter(e=>e.name===name)
      if (thisWeek.length===0||all.length<2) return null
      const weekBest = Math.max(...thisWeek.map(e=>e.bestSet))
      const prevBest = all.filter(e=>!isSameWeek(e.date,new Date())).slice(-1)[0]?.bestSet
  ...
  const weightLost = profile?.weight && currentWeight ? Math.max(0,Math.round((profile.weight-currentWeight)*10)/10) : 0
  ...
  const shareStats = { consistencyScore, avgCalories, avgProtein, avgSteps, sessionsThisWeek:thisWeekSessions.length, topImprovement }
  ...
  return (
    <div style={{paddingTop:52,paddingBottom:24,background:'#080808',minHeight:'100vh'}}>
      ...
      {showShare && <ShareModal profile={profile} stats={shareStats} onClose={()=>setShowShare(false)}/>}

      {/* Header */}
      <div style={{padding:'0 20px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div>
          <h1 style={{fontSize:26,fontWeight:700,letterSpacing:'-.5px'}}>Progress</h1>
      ...
      {activeTab==='overview'&&(
        <>
          {/* Weekly stats row — like the demo */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,margin:'0 20px 14px',overflow:'hidden'}}>
            {[
              ...
              {label:'Calories',val:avgCalories,sub:'avg/day',color:'#EF4444'},
              {label:'Steps',val:avgSteps>=1000?`${Math.round(avgSteps/100)/10}K`:avgSteps,sub:'avg/day',color:'#22C55E'},
              {label:'Sessions',val:sessions.length,sub:'total',color:'#A855F7'},
              ...
              <div key={label} style={{padding:'14px 8px',textAlign:'center',borderRight:i<arr.length-1?'1px solid #141414':'none'}}>
                ...
                <div style={{height:2,background:'#111',borderRadius:1,margin:'6px auto 0',width:'70%',overflow:'hidden'}}>
                  <div style={{height:'100%',background:color,borderRadius:1,width:'60%'}}/>
                </div>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
              <div>
                ...
                <div style={{display:'flex',alignItems:'baseline',gap:6}}>
                  ...
                  <span style={{fontSize:14,color:'#333',fontWeight:500}}>/100</span>
                </div>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:6,textAlign:'right'}}>
                ...
                <p style={{fontSize:11,color:'#333'}}>Nutrition {Math.round(nutritionDays/7*100)}%</p>
                <p style={{fontSize:11,color:'#333'}}>Steps {Math.round(stepsDays/7*100)}%</p>
              </div>
            </div>
          ...
          <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
              <div>
                <p style={{fontSize:14,fontWeight:700}}>Calories Consumed</p>
          ...
          {/* Bottom 3 metrics — like demo */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,padding:'0 20px',marginBottom:14}}>
            {[
              {label:'Consistency',val:`${consistencyScore}%`,color:'#FF5A1F',pct:consistencyScore,sub:`${thisWeekSessions.length} of ${profile?.training_days_per_week?.split(' ')[0]||5} sessions`},
              {label:'Streak',val:`${sessions.length}`,color:'#F59E0B',pct:Math.min(sessions.length*5,100),sub:'sessions logged',icon:'🔥'},
          ...
          {/* Training breakdown */}
          <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
            <p style={{fontSize:14,fontWeight:700,marginBottom:14}}>Training Breakdown</p>
            ...
            {Object.keys(typeCount).length===0&&<p style={{color:'#333',fontSize:13}}>No sessions logged yet</p>}
          </div>
        </>
      )}
        ...
        <>
          ...
          <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,overflow:'hidden'}}>
            <div style={{padding:'14px 16px',borderBottom:'1px solid #111',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <p style={{fontSize:15,fontWeight:700}}>Strength Progress</p>
            ...
            {strengthPRs.length===0?(
              ...
              strengthPRs.map(({name,rm,delta,count})=>(
                <div key={name} style={{padding:'14px 16px',borderBottom:'1px solid #0d0d0d',display:'flex',alignItems:'center',gap:12,cursor:'pointer',background:selectedExercise===name?'#FF5A1F10':'transparent'}}
          ...
          {selectedExercise&&exerciseChartData.length>0&&(
            <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
                <div>
                  <p style={{fontSize:14,fontWeight:700}}>{selectedExercise}</p>
              ...
              <div style={{marginTop:12,padding:'10px 14px',background:'#111',borderRadius:12}}>
                <p style={{fontSize:13,color:'#888',lineHeight:1.6}}>
                  {!selectedImprovement?'Not enough data yet — log more sessions with this exercise.'
                  :selectedImprovement.pct>20?`🔥 ${selectedImprovement.pct}% improvement. That's actual progress. Keep pushing.`
                  :selectedImprovement.pct>5?`📈 ${selectedImprovement.pct}% progress. Solid. Add weight next session.`
                  :selectedImprovement.pct>0?`${selectedImprovement.pct}% improvement. Technically progress. Push harder.`
                  :selectedImprovement.pct===0?`Same weight, same reps. Progressive overload is a thing. Look it up.`
                  :`You're getting weaker at ${selectedExercise}. Either you're injured or your effort is. Which is it?`}
                </p>
              </div>
            </div>
          )}
      ...
      {activeTab==='charts'&&(
        <>
          ...
          {activeChart==='calories'&&(
            <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:14}}>
                <div><p style={{fontSize:14,fontWeight:700}}>Calories (7 days)</p><p style={{fontSize:11,color:'#444',marginTop:2}}>Daily intake</p></div>
          ...
          {activeChart==='volume'&&(
            <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:14}}>
                <div><p style={{fontSize:14,fontWeight:700}}>Weekly Sessions</p><p style={{fontSize:11,color:'#444',marginTop:2}}>Last 8 weeks</p></div>
          ...
          {activeChart==='weight'&&(
            <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                <div><p style={{fontSize:14,fontWeight:700}}>Weight Trend</p><p style={{fontSize:11,color:'#444',marginTop:2}}>{weightLogs.length} entries</p></div>
      ...
      {activeTab==='body'&&(
        <>
          ...
          <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
              <p style={{fontSize:15,fontWeight:700}}>Weight Journey</p>
              <div style={{display:'flex',gap:8}}>
                <input type="number" placeholder="kg" value={newWeight} onChange={e=>setNewWeight(e.target.value)}
                  style={{width:60,background:'#111',border:'1px solid #222',borderRadius:9,padding:'6px 8px',color:'#fff',fontSize:13,outline:'none',textAlign:'center'}}/>
                <button onClick={logWeight}
            ...
            {profile?.weight&&profile?.goal_weight&&(
              <>
                ...
                <p style={{fontSize:11,color:'#333'}}>
                  {weightLost>0?`${weightLost}kg lost so far`:'Start logging weight to track progress'}
                </p>
              </>
            )}
          </div>
          ...
          {weightData.length>0&&(
            <div style={{margin:'0 20px 14px',background:'#0a0a0a',border:'1px solid #141414',borderRadius:18,padding:16}}>
              <p style={{fontSize:14,fontWeight:700,marginBottom:14}}>Weight trend</p>
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={weightData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#111" vertical={false}/>
                  <XAxis dataKey="date" tick={{fill:'#333',fontSize:10}} axisLine={false} tickLine={false}/>
