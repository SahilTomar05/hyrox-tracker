...
function getScore(exs, rpe, dur) {
  ...
  s=Math.min(s,100)
  if (s>=85) return {isRecovery:false,score:s,emoji:'🔥',color:'#22C55E',msg:`${s}/100. Actually impressive. Keep that energy tomorrow.`}
  ...
  return {isRecovery:false,score:s,emoji:'💀',color:'#EF4444',msg:`${s}/100. That was... something. Add weight, add sets, add effort. Come back serious.`}
}
...
function getWeekDates(){
  ...
  return Array.from({length:7},(_,i)=>{const d=new Date(today);d.setDate(today.getDate()-(6-i));return d})
}
...
export default function Training({ session, profile }) {
  ...
  function addWithSets(ex,n) {
    setExercises(p=>[...p,{id:Date.now(),...ex,fieldType:'strength',sets:Array.from({length:n},()=>({reps:'',weight:''}))}])
    setSetsPicker(null)
  }
  ...
  function openForm(){setShowForm(true);setExpandedId(null);setTimeout(()=>searchRef.current?.focus(),150)}
          ...
          <div style={{padding:'14px 16px',background:'#0a0a0a',borderBottom:'1px solid #111',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:20}}>
            <div>
              <p style={{fontSize:16,fontWeight:700}}>
                {step==='search'?'Add Exercises':'Review & Log'}
              </p>
              {detectedType
                ...
                :<p style={{fontSize:11,color:'#2a2a2a',marginTop:2}}>{step==='search'?'Search or pick a category':'Fill in your sets below'}</p>
              }
            </div>
          ...
          {step==='search'&&(
            <div style={{padding:16,display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style={{display:'flex',gap:6,overflowX:'auto',scrollbarWidth:'none',paddingBottom:2}}>
                ...
                {CATS.map(cat=>{
                  const m=CAT_META[cat];const on=activeCat===cat
                  return (
              ...
              {!query&&!activeCat&&(
                <div style={{textAlign:'center',padding:'24px 0'}}>
                  ...
                  <p style={{color:'#333',fontSize:14,fontWeight:500}}>Type to search exercises</p>
                  <p style={{color:'#222',fontSize:12,marginTop:4}}>or tap a category above to browse</p>
                </div>
              )}
                    ...
                    filtered.map(ex=>{
                      ...
                      return (
                        <button key={ex.name} onClick={()=>tapResult(ex)}
                          style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',borderRadius:12,border:`1px solid ${isAdded?m.color+'30':'#141414'}`,background:isAdded?m.color+'10':'#0d0d0d',cursor:'pointer',transition:'.15s',textAlign:'left',width:'100%',boxSizing:'border-box'}}>
              ...
              {/* Added summary chips */}
              {exercises.length>0&&(
                <div style={{background:'#0d0d0d',borderRadius:14,border:'1px solid #1a1a1a',padding:'10px 12px'}}>
                  <p style={{fontSize:10,color:'#444',fontWeight:700,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:8}}>Added · {exercises.length}</p>
                  <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                    {exercises.map(ex=>{
                      const m=CAT_META[ex.cat]||CAT_META['Custom']
                      return (
                        <div key={ex.id} style={{display:'flex',alignItems:'center',gap:5,padding:'5px 10px',borderRadius:20,background:m.color+'15',border:`1px solid ${m.color}30`}}>
              ...
              <button onClick={()=>exercises.length>0&&setStep('log')} disabled={!exercises.length}
                style={{width:'100%',background:exercises.length?'linear-gradient(135deg,#FF5A1F,#FF8C42)':'#0d0d0d',border:'none',borderRadius:14,padding:15,color:exercises.length?'#fff':'#2a2a2a',fontSize:15,fontWeight:700,cursor:exercises.length?'pointer':'not-allowed',boxShadow:exercises.length?'0 4px 20px rgba(255,90,31,.25)':'none',transition:'.2s'}}>
                {exercises.length?`Continue — Log ${exercises.length} exercise${exercises.length>1?'s':''} →`:'Add exercises above to continue'}
              </button>
            </div>
          )}
          ...
          {step==='log'&&(
            <div style={{padding:16,display:'flex',flexDirection:'column',gap:14,maxHeight:'70vh',overflowY:'auto',scrollbarWidth:'none'}}>
              ...
              {exercises.map(ex=>{
                ...
                return (
                  <div key={ex.id} style={{background:'#0d0d0d',border:`1px solid ${m.color}15`,borderRadius:16,padding:14}}>
                    ...
                    {/* STRENGTH — vertical set rows, no overflow */}
                    ...
                    {ex.fieldType==='timed'&&(
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                        ...
                        <div>
                          ...
                          <input placeholder="30 min" value={ex.duration} onChange={e=>updEx(ex.id,'duration',e.target.value)} style={{...inp,width:'100%',boxSizing:'border-box'}}/>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <button onClick={save} disabled={saving}
                ...
                {saving?'Saving...':`Save ${detectedType||'Session'} · ${exercises.length} exercise${exercises.length>1?'s':''}`}
              </button>
